...
export const load: LayoutServerLoad = async ({ fetch, locals, url }) => {
	...
	// Gate onboarding butuh tahu apakah user sudah punya event. Ambil lewat adapter
	// API yang sama dengan halaman app lain, bukan tabel Supabase langsung: event
	// sekarang dimiliki backend Go, jadi dua sumber data bisa tidak sinkron.
	let hasEvents = true;
	try {
		const events = await listMyEvents({ fetch, accessToken: session.access_token });
		hasEvents = events.length > 0;
	} catch {
		// Saat API sedang tidak tersedia, jangan blokir user lama dengan onboarding
		// yang salah. User baru tetap diarahkan lewat redirect register langsung.
	}
