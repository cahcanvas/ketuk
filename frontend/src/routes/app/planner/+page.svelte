...
<div class="mx-auto max-w-5xl">
	{#if !data.eventId}
		...
		{#if data.events.length === 0}
			...
			<EmptyState
				...
				description="Buat acara dulu sebelum mengatur planner."
			>
					{#snippet action()}
						<Button href="/app/undangan/baru">Buat Undangan</Button>
					{/snippet}
				</EmptyState>
			</div>
		{:else}
			<div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{#each data.events as event (event.id)}
					<a href="/app/planner?event={event.id}" class="block">
						<EventCard {event} />
					</a>
				{/each}
			</div>
	...
	{:else}
		<div class="flex flex-col gap-1">
			<a href="/app/planner" class="text-sm text-coffee-500 hover:text-coffee-900">
		...
		{:else if data.summary}
			<div class="mt-6 grid gap-4 sm:grid-cols-2">
				<a
					href="/app/planner/budget?event={data.eventId}"
				...
				<a
					href="/app/planner/checklist?event={data.eventId}"
					class="group flex flex-col gap-3 rounded-2xl border border-coffee-100 bg-white p-6 transition-all hover:-translate-y-0.5 hover:border-coffee-200 hover:shadow-md"
				>
			...
			<div class="mt-4">
				<a
					href="/app/planner/timeline?event={data.eventId}"
